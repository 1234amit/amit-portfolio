  ...
  return (
    <main className="bg-black text-gray-100 px-6 md:px-10 lg:px-16 xl:px-24 py-12">
      <div className="mx-auto max-w-6xl grid gap-12 md:grid-cols-[1.05fr_.95fr]">
        <section>
          <p className="text-emerald-400 font-medium tracking-wider">Full-Stack Software Developer</p>
          <h1 className="mt-2 text-4xl md:text-5xl font-extrabold leading-tight">I design, build, and ship end-to-end web applications</h1>
          <p className="mt-4 max-w-prose text-gray-300">I build modern, fast, and scalable products across the stack. From accessible UIs in React/Next.js to robust APIs in Node, Spring Boot, or ASP.NET, and production-ready databases with MySQL, PostgreSQL, and MongoDB.</p>
          ...
          <div className="mt-10 flex flex-wrap items-center gap-4">
            ...
            <Button variant="outline" className="rounded-full border-emerald-500/40 text-emerald-300 hover:bg-emerald-500/10 gap-2">
        ...
        <aside className="flex flex-col items-center md:items-end">
          ...
          <div className={`${styles.panel} mt-8 w-full max-w-sm rounded-2xl p-6`}>
...
function StackCard({ title, items }) {
  return (
    <div className={`${styles.panel} rounded-2xl p-5`}>
      <div className="text-sm font-medium text-gray-300">{title}</div>
      <div className="mt-4 flex flex-wrap gap-2">
        {items.map(i => (
          <span key={i} className="rounded-full border border-emerald-500/30 bg-emerald-500/10 px-3 py-1 text-xs text-emerald-300">{i}</span>
        ))}
      </div>
    </div>
  )
